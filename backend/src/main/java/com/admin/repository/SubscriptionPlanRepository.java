...
public class SubscriptionPlanRepository {
    ...
    public Optional<SubscriptionPlanDetails> findById(String id) {
        return jdbcTemplate.query(
                        """
                        SELECT sp.id, sp.plan_code, sp.plan_name, sp.billing_period, sp.price_amount,
                               sp.currency, sp.limits_json, sp.features_json, sp.status,
                               sp.created_at, sp.updated_at,
                               (SELECT COUNT(DISTINCT ts.tenant_id)
                                FROM tenant_subscriptions ts
                                WHERE ts.subscription_plan_id = sp.id
                                  AND ts.status IN ('ACTIVE', 'TRIAL')) AS active_tenants_count
                ...
                SELECT sp.id, sp.plan_code, sp.plan_name, sp.billing_period, sp.price_amount,
                       ...
                       (SELECT COUNT(DISTINCT ts.tenant_id)
                        ...
                        WHERE ts.subscription_plan_id = sp.id
                          AND ts.status IN ('ACTIVE', 'TRIAL')) AS active_tenants_count
                FROM subscription_plans sp
                WHERE (? = '' OR UPPER(sp.plan_code) LIKE UPPER(?) OR UPPER(sp.plan_name) LIKE UPPER(?))
                  AND (? = '' OR sp.status = ?)
                ORDER BY sp.created_at DESC, sp.plan_code ASC
                LIMIT ? OFFSET ?
                """,
                (resultSet, rowNumber) -> mapDetails(resultSet),
                search,
    ...
    private SubscriptionPlanDetails mapDetails(java.sql.ResultSet resultSet)
        ...
        return new SubscriptionPlanDetails(
                ...
                resultSet.getInt("active_tenants_count"));
    }
